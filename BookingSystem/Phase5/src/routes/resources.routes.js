...
router.post("/", resourceValidators, async (req, res) => {
  ...
  try {
    ...
    await logEvent({
      actorUserId,
      message: `RESOURCE CREATED ✅: name=${resourceName}`,
      entityType: "resource",
      entityId: resourceId,
    });
    ...
    if (err && err.code === "23505") {
      ...
      await logEvent({
        actorUserId,
        message: `RESOURCE CREATION BLOCKED ⛔: DUPLICATE NAME FOUND`,
        entityType: "resource",
        entityId: null,
      });
      ...
      return res.status(409).json({
        ...
        details: "A resource with the same name already exists. Please enter another name.",
      });
    }
