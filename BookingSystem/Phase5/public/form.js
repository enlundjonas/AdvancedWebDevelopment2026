...
function buildValidationMessage(errors) {
  ...
  return `Unable to create resource. Please check the following:\n\n${lines.join("\n")}`;
}
...
async function onSubmit(event) {
  ...
  const resourcePriceUnit = selectedUnit;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 409) {
        ...
        showFormMessage("info", `ERROR: Resource duplicate detected.\n\n${msg}`);
        return;
      }
    ...
    const msgLines = [];
    msgLines.push('Resource added succesfully.');
    msgLines.push(`Name: ${body?.data?.name ?? ""}`);
    msgLines.push(`Price: ${resourcePrice}€/${resourcePriceUnit}`);
