...
function createResourceDescriptionInput(container) {
  ...
  // Base Tailwind styling (single source of truth)
  input.className = `
    mt-2 h-24 w-full rounded-2xl border border-black/10 bg-white
    px-4 py-3 text-sm outline-none
    focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/30
    transition-all duration-200 ease-out
  `;
...
function isResourceDescriptionValid(value) {
  ...
  // Allowed: letters, numbers, Finnish letters, and space (based on your current regex)
  const allowedPattern = /^[\w\s.,!?:;äöåÄÖÅ-]+$/;
...
function updateFormButtonState() {
  if(!resourceNameInput || !resourceDescriptionInput || !priceInput) return;
...
function attachPriceValidation(input){
  const update = () => {
    const raw = input.value;
...
attachPriceValidation(priceInput);
